...
namespace VNM_ViSport_Charging_SpamSms.Library
{
    public class SMS_MTController
    {
        ...
        #region Proccess MT
        public static ViSport_S2_Registered_SpamSms_UserInfo initInfo(DataRow row)
        {
            ViSport_S2_Registered_SpamSms_UserInfo info = new ViSport_S2_Registered_SpamSms_UserInfo();
            try
            {
        ...
        private void EnQueue(DataTable tbl)
        {
            ...
            if (tbl != null && tbl.Rows.Count > 0 && MSMProccess.MT_PROC_QUE.Count < 100)            
            {
                ...
                foreach (DataRow row in tbl.Rows)
                {
                    try
                    {
                        ViSport_S2_Registered_SpamSms_UserInfo info = initInfo(row);                        
