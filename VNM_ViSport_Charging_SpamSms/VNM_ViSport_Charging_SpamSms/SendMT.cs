...
namespace VNM_ViSport_Charging_SpamSms
{
    public partial class SendMT : ServiceBase
    {
        ...
        private TQueue<ViSport_S2_Registered_SpamSms_UserInfo> queue = new TQueue<ViSport_S2_Registered_SpamSms_UserInfo>();        
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                this.workItemsProducerThread = new Thread(this.WorkItemsProducer);
            ...
            {
                _logger.Error(string.Concat("Error - VNM_ViSport_Charging_SpamSms.SendMT.Onstart: ", ex.Message));
                _logger.Error(string.Concat("Error - VNM_ViSport_Charging.SpamSms.SendMT.Onstart: ", ex.StackTrace));
            }            
        }        
        ...
        private void WorkItemsProducer()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        ViSport_S2_Registered_SpamSms_UserInfo info = MSMProccess.MT_PROC_QUE.Dequeue();
                        if (info != null)
                        {
                            WorkItemCallback workItemCallback = DoSomeWork1;
                            workItemsGroup.QueueWorkItem(workItemCallback, info);
                        }
                        else
                        {
                            _logger.Error("Dequeue ViSport_S2_Registered_SpamSms_UserInfo IS NULL");
                        }
                    }
                    else
                    {
        ...
        private object DoSomeWork1(object obj)
        {
            try
            {
                MSMProccess.ChargeUser((ViSport_S2_Registered_SpamSms_UserInfo)obj);               
            }
            catch (Exception ex)
            {
